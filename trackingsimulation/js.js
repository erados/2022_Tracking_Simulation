...
function calculate_dist_square(){
	for (var i = 0; i < n; i++){
		for (var j = i + 1; j < n; j++){
			if(i != j){
				dist_square[i][j] = real_points[i].calculate_distance_square(real_points[j]);
...
function calculate_gradiant(){
	for(var i = fixed; i < n; i++){
		...
		for(var j = 0; j < n; j++){
			if(i != j){
				var common_term = 4 * (points[i].calculate_distance_square(points[j]) - dist_square[i][j]);
...
function calculate_error(){
	...
	for(var i = 0 ; i < n; i ++){
		for(var j = i + 1 ; j < n; j ++){
			var temp =  points[i].calculate_distance_square(points[j]) - dist_square[i][j];
			error += temp * temp;
		}
	}
